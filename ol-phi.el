...
repository NAME."
  `(lambda (id _)
     ,(format "Autogenerated Org link open function for repository %s" name)
     (phi-find-note id ,name)))

(defun make-org-link-phi-store-function (name)
  "Create the link store function for an Org link type
corresponding to repository NAME."
  `(lambda ()
     ,(format "Autogenerated Org link store function for repository %s" name)
     (when (and (phi-mode)
